...
class Decoder(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False, has_residual=False):
        ...
        # self.block_five_upDecoder = upsampling(n_filters * 16, n_filters * 8, normalization=normalization)
    ...
    def forward(self, features):
        ...
        # x5_up = self.block_five_upDecoder(x5)
...
class VNet_prob(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, outdim=128,normalization='none', has_dropout=False,
                 has_residual=False):
        super(VNet_prob, self).__init__()

        self.encoder = Encoder(n_channels, n_classes, n_filters, normalization, has_dropout, has_residual)
        self.decoder = Decoder(n_channels, n_classes, n_filters, normalization, has_dropout, has_residual)

        self.representation = nn.Sequential(
            nn.Conv3d(n_filters, outdim, (3, 3, 3), padding=(1, 1, 1)),
            nn.BatchNorm3d(outdim),
            nn.ReLU(inplace=True),
            nn.Conv3d(outdim, outdim, (1, 1, 1))
        )

    def forward(self, input):
        features = self.encoder(input)
        out_seg, x9 = self.decoder(features)
        representation = self.representation(x9)
        return out_seg, representation, x9  # 4, 16, 112, 112, 80
...
if __name__ == '__main__':
    ...
    model = VNet_prob(n_channels=1, n_classes=1, outdim=128,normalization='batchnorm', has_dropout=False)
