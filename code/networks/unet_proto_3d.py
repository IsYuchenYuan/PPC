...
class UNetProto(nn.Module):
    def __init__(
        ...
        self.uncer_head = Uncertainty_head_inverse(in_feat=16, out_feat=embed_dim)
    ...
    def forward(
        ...
        inverse_sigma_sq = self.uncer_head(feature2d)  # B*D, dim, H, W
        sigma_sq = 1 / inverse_sigma_sq

        # log_sigma_sq = self.uncer_head(feature2d)  # B*D, dim, H, W
    ...
    def prototype_update(self,
        ...
        return self.prototype_learning(
            mu,
            sigma_sq,
            label_onehot,
            mask_,
    ...
    def initialize(self,
        ...
        for i in range(num_segments):  # num_cls
            ...
            # prototype computing
            with torch.no_grad():
                proto_sigma_ = 1 / torch.sum((1 / sigma[valid_pixel.bool()]), dim=0, keepdim=True)  # 1, dim
    ...
    def prototype_learning(
        ...
        :param mu: the mean of the probabilistic representation in the current batch (B*D,dim,H,W)
        :param label: the one-hot label of the batch data (B*D,C,H,W)
        :param mask: indicates the high prob pixels of the prediction (B*D,1,H,W)
        :param sigma: the variance of the probabilistic representation in the current batch (B*D,dim,H,W)
        :return:
        """
        ...
        # num_points_max = torch.max(torch.sum(valid_pixel_all,dim=[0,2,3])).long().item()
        # print("minimum sample points: {}".format(num_points_min))
        # print("maximum sample points: {}".format(num_points_max))
        for i in range(num_segments):  # num_cls
            valid_pixel = valid_pixel_all[:, i] # B, H, W
            if valid_pixel.sum() == 0:
                # continue
                ...
                if self.momentum:
                    ...
                    proto_sigma_ = torch.full((1, mu.size(-1)), 1e+32).cuda()
                    proto_mu_ = torch.zeros((1, mu.size(-1))).cuda()

            else:
                # new prototype computing
                with torch.no_grad():
                    # 1: conditional independence assumption
                    # proto_sigma_ = 1 / torch.sum((1 / sigma[valid_pixel.bool()][:num_points_min]), dim=0, keepdim=True) # 1, dim
                    # proto_mu_ = torch.sum((proto_sigma_ / sigma[valid_pixel.bool()][:num_points_min]) \
                    #                       * mu[valid_pixel.bool()][:num_points_min], dim=0, keepdim=True) # 1, dim

                    proto_sigma_ = 1 / torch.sum((1 / sigma[valid_pixel.bool()]), dim=0,
                                                 keepdim=True)  # 1, dim
                    proto_mu_ = torch.sum((proto_sigma_ / sigma[valid_pixel.bool()]) \
                                          * mu[valid_pixel.bool()], dim=0, keepdim=True)  # 1, dim
